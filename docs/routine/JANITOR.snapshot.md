...
description: Every 4 h — archive the feedback routine's finished Desktop sessions by policy (idle ticks and working runs beyond the 3 newest, once quiet for 2 h) using scripts/routine-janitor-scan.mjs; never deletes, never touches a running session.
---
...
4. Report exactly one line and stop:
   `Janitor: archived <n> (<idle> idle, <working> working) · deferred <f> (< 2 h quiet) · kept <k> working, <r> running · <d> idle ticks await interactive deletion · next sweep in 4 h`
   If the script fails, report `Janitor: scan failed — <first error line>` and stop; do not archive by guesswork.

Policy (operator, 2026-09-17/18): idle ticks (< 180 s of activity) are archived; working runs keep the 3 newest un-archived and older ones are archived; a run touched within 3 minutes is running and untouched; deletion of idle ticks older than 1 h needs a click and happens only interactively. Age gate (2026-09-18): archive calls on sessions quiet for less than 2 h drew a consent card even from this bypass session (18:10 run: 7 calls, results only after the operator's clicks) while a 14-h-old tick archived in 6 s without one — the script therefore defers fresh candidates; if a call still blocks, the report line says so and the rest of the sweep is not retried. Details: .claude/skills/routine-janitor/SKILL.md, .claude/deep-knowledge/scheduled-tasks.md.