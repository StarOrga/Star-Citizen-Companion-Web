...
| ~~`weaponParams.ammoContainerRecord` is `null` on **all 430**~~ **CLOSED (feedback #237, schema v6)** — the field really is null on every ship weapon and all 188 countermeasure launchers, but it was never the link: the round sits on the weapon entity's OWN `SAmmoContainerComponentParams.ammoParamsRecord` (a cross-file Reference stub to `AmmoParams.<class>`), with `maxAmmoCount` on the same struct. FPS weapons point `ammoContainerRecord` at a separate magazine entity carrying the same struct (second hop). VERIFIED LIVE 4.10 (2026-09-18): `CNOU_Nomad_CML_Flare` → `BEHR_Flare` × 48, `CNOU_Nomad_CML_Chaff` → `TALN_Chaff` × 5, `RSI_Perseus_BallisticCannon_B_S8` → `RSI_Perseus_BallisticCannon_S8_AMMO` × 900. Coverage 188/188 launchers, 195/196 ship guns + rocket pods (50 of them NOT `<class>_AMMO`), 390/395 FPS weapons (binoculars), all beams (placeholder rounds); mining lasers carry no round | `weaponParams.ammoClassName` / `ammoGuid` / `ammoCapacity` on every weapon that fires a round. The web prefers `ammoClassName` and keeps `<weaponClass>_AMMO` only as the fallback for pre-v6 builds; the countermeasure block shows rounds carried + the decoy's IR / EM / cross-section |
...
interface Weapon extends BaseEntity {
  ...
  // + (schema v6) the round the weapon fires, read off the entity's own
  //   SAmmoContainerComponentParams (FPS: off the magazine entity behind
  //   ammoContainerRecord); absent — never guessed — when there is none:
  //   ammoClassName: string   — AmmoParams class, the key of ammunition/*.json
  //   ammoGuid: string        — that record's GUID
  //   ammoCapacity?: number   — maxAmmoCount, only when 0 < n < int32 max
...
```jsonc
{
  ...
  "schema_version": 6,                   // 1 = wave-1, 2 = ship signature stats +
                                         // weapon fireRate, 3 = resource network
                                         // (power/coolant/signature) + weapon stats +
                                         // ship hull/armour/cargo/career, 4 = items
                                         // keep their resource network, 5 = tractor
                                         // beam reach/pull, 6 = weapon → round link
                                         // (ammoClassName/ammoGuid/ammoCapacity)
