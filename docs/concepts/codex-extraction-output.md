...
| No **ship hull HP**; per-hull damage multipliers were unreachable | ship payloads carry no health struct (still open). The per-hull `ARMR_<ship>` item WAS stat-less — **partially closed (PR A)**: it carries `SCItemVehicleArmorParams` (signature multipliers, per-channel `damageMultiplier`, `armorPenetrationResistance`, `armorDeflection`), gated on the struct being present rather than on `AttachDef.Type` (the vehicle-armor vocabulary doesn't overlap the FPS `Char_Armor_*` one `_ARMOR_TYPES` already covered); shields still expose only `SHealthComponentParams.DamageResistances.IgnoreMeleeDamage` | the **Defence** panel shows the shield pool, regen, delays and distortion pool, and can now also show the hull's per-channel damage multipliers/penetration/deflection; hull HP itself still renders as `—` with an explicit note rather than a fabricated number |
| `payload.flight` is **all-null on every ship** (0 of 314 carry `scmSpeed`) | Confirmed (PR A, against LIVE 4.9.0 `CNOU_Nomad`): `VehicleComponentParams.vehicleDefinition` is a bare **game-file path string** (`Scripts/Entities/Vehicles/Implementations/Xml/CNOU_Nomad.xml`), not a DCB record reference — so this is NOT a `record_by_id` hop like the armour-multiplier gap below. Following it needs a P4K-relative XML file open + a second parser (IFCS/`SCItemFlightControllerParams` live in that XML, not in `Game2.dcb`), out of scope for PR A | the ship page's **Hull & flight** block renders the rows as `—`; closing this gap is a separate XML-parsing task, not a schema/record-graph fix |
...
interface Ship extends BaseEntity {
  ...
  // Whitelist-only ship stats (PR A) — CLOSED for the signature system, see
  // §0b: `SSCSignatureSystemParams` (IR/EM/cross-section), same struct-keyed
  // shape as a component's `stats`. Deliberately an allowlist, not the
  // item/component blacklist — a ship's Components list is far larger and
  // noisier. Absent entirely when the whitelist finds nothing.
