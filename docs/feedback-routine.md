...
window only stamps the heartbeat, see "The gate") — it fires
...
Runs can overlap (a run can outlast the tick that started it; the gate's run
lock makes a second run stand down, see "The gate"). The atomic claim
...
- **`processed_at < now() - interval '30 minutes'`** protects a *currently
  ...
  check on the item's worktree (see "The gate"), because a bare timer reopened
...
| `review_reply_pending` | `true` from the `shipped` UPDATE until the ✅/⏳ review reply is actually posted (migration `20260913131500`); query (f) picks these up first thing in a working run — see "Review reply as a checkpoint" |
...
| `state`        | `idle` · `running` (run lock held) · `paused` (usage brake) — see "The gate" |
