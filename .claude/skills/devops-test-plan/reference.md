...
echo "=== GH Release page (auth via gh) ===" && gh release view data-uploader-v$(gh release list --limit 1 --json tagName --jq '.[0].tagName' | sed 's/data-uploader-v//') --json name,assets --jq '{name, asset_count: (.assets | length)}'
...
| GH Release | JSON with `asset_count >= 1` | Tagged release exists, assets uploaded by `data-uploader-build` workflow |
...
`curl https://github.com/Jerry0022/Star-Citizen-Companion-Web/releases/tag/data-uploader-v0.3.3`
...
Since 2026-05-24 (issue [#7](https://github.com/Jerry0022/Star-Citizen-Companion-Web/issues/7)
...
After every `data-uploader-v*` release, probe the asset URL UNAUTHENTICATED to
verify end-users can actually download:

```bash
TAG=data-uploader-v$(gh release list --limit 1 --json tagName --jq '.[0].tagName' | sed 's/data-uploader-v//')
VERSION=$(echo "$TAG" | sed 's/^data-uploader-v//')
curl -sIL "https://github.com/StarOrga/Star-Citizen-Companion-Binaries/releases/download/$TAG/data-uploader-setup-$VERSION-x64.exe" \
...
match the source repo's `data-uploader-v*` tag), or the
