...
| `20260904040000_feedback_attachment_types.sql` | Attachment types on the `feedback-images` bucket (admin feedback 312a4acc): `allowed_mime_types` cleared to NULL and the per-role rule moved into `feedback_images_owner_upload` — everyone may still upload an image, a NON-image object may only be written by `public.is_admin()`. Rule keys on the object name's extension, because `metadata` is not populated when the WITH CHECK runs. 5 MB per-object ceiling unchanged (the composer now pre-checks it) |
...
| `20260906140000_admin_feedback_summary.sql` | `admin_feedback.summary` — the one-line title the admin-feedback routine writes for a topic when it claims it (feedback `d08f1983`), capped at 120 chars by `admin_feedback_summary_len`. Nullable and additive: the board falls back to the body-derived title (`displayTitle` in `src/app/admin/feedback/feedback.types.ts`), so pre-existing rows are unchanged and no backfill runs. Admin-only, like `seq` — deliberately NOT added to `public.my_feedback`, so that view keeps its load-bearing `revoke all … / grant select` pair untouched |
| `20260907220000_admin_feedback_complex.sql` | `admin_feedback.complex` — the admin's "work this one two reasoning steps higher" opt-in (feedback `423e5130`), ticked in the new-topic composer next to the send button; the rule it triggers lives in `docs/feedback-routine.md`. `boolean not null default false`, so the ALTER is metadata-only and every pre-existing row is an ordinary topic — no backfill. **No policy or grant change**: `admin_feedback_insert` enumerates no columns and the table-level `grant insert` covers new ones. The one existing object touched is the BEFORE INSERT guard `admin_feedback_normalize_user_insert`, recreated verbatim + `new.complex := false` so a non-admin cannot self-declare how much thinking the routine spends on their topic (hardening; the admin UPDATE path stays open, so an admin may still raise it at triage) |
| `20260913121500_routine_heartbeat_run_state.sql` | `routine_heartbeat.next_run_at` / `state` (`idle`·`running`·`paused`, checked) / `run_started_at` / `run_finished_at` — the gate script (`scripts/routine-gate.mjs`, concept 2026-09-13) stamps when the next tick is due and whether a run holds the lock; the admin panel derives green/accent/amber/red from these instead of a fixed 45-min window. Additive, idempotent, no policy change (the admin-only SELECT policy covers the new columns; writes stay service-role only) |
| `20260913131500_admin_feedback_review_reply_pending.sql` | `admin_feedback.review_reply_pending` — set true in the same UPDATE that marks a topic `shipped`, cleared when the ✅/⏳ review reply is posted; query (f) of the routine picks these up first thing in a run so a run that dies during deploy verification never leaves a shipped topic without its reply. Partial index on `shipped_at where review_reply_pending`. Not in `my_feedback` |
| `20260914160000_concept_pages.sql` | `concept_pages` — interactive concept pages hosted ON the website (admin feedback #224, `/konzept/<id>`): `html` (the full concept document), `decisions` (the engine's `/decisions` payload), `draft` (note mirror `{recovered}`), `submitted_at` / `processed_at` (stamped by the page / by the routine's `concept-read --mark-processed`), `reload_counter` (bumped by `concept-publish --id`, the page reloads when it grows), `feedback_id → admin_feedback` (cascade). Admin-only SELECT, **no client insert/update/delete policy** — written by the `concept-page` edge function (service role, behind its HMAC ticket) and by `scripts/routine-gate.mjs` (Management API). `anon` has no grant. Additive, idempotent |
...
| `20260904020000_social_moderation_sharing.sql` | Phase 2 of cf0ddf7d, three things. **(1) Moderation** — `profiles.suspended_at`/`suspended_until`/`suspension_reason` (current state, denormalized because `is_approved()` runs in a RESTRICTIVE policy on nearly every table and must stay one indexed lookup) plus the `moderation_actions` ledger (`warning`\|`suspension`\|`unsuspension`). **`is_approved()` now also means "not suspended"**, so a suspension closes every existing `*_approved_gate` at once with no policy churn. It also **widens `profiles_role_write_guard`** to freeze the three new columns — `profiles_self_update` (00001) has no column list, so otherwise a suspended account clears its own suspension with one PATCH. Admin RPCs `warn_user()` / `suspend_user()` / `resolve_reports_for_user()` go through `moderation_target()`, which refuses self, admins and `protected_admins`; `unsuspend_user()` deliberately does **not** (that guard is right for imposing a sanction and a trap for lifting one — promote a suspended viewer to admin and the suspension would become unliftable), and `resolve_reports_for_user()` refuses only self (an admin must be able to dismiss a bogus report against another admin). `is_suspended(uuid)` is **not granted to `authenticated`** — an arbitrary-target predicate on `/rpc/` is a moderation-state oracle; every real caller is SECURITY DEFINER. `suspend_user()` also DELETEs `auth.sessions` for the target — **best effort, wrapped in an exception block**, since the `auth` schema is owned by `supabase_auth_admin`; the authoritative lockout is the RLS one. It deliberately does NOT set `auth.users.banned_until`: the suspended user must still be able to authenticate long enough to be TOLD WHY (`my_account_status()`, not gated on `is_approved()`, else the notice would be unreadable to the only person who needs it). **(2) Loadout sharing** — `loadout_shares`, one row per share, either `token` (public link) or `shared_with` (friend), never both. `hangar_role_loadouts` keeps its self-only RLS: `get_shared_loadout(token)` (granted to **anon** — the only hangar data anon can reach) and `list_loadouts_shared_with_me()` (re-checks `are_friends()` on every read, so un-friending revokes with no cleanup pass) are the only non-owner paths. **(3) 7-day friend-request expiry** — enforced in `list_my_friend_edges()` (which gains `expires_at`) and `respond_friend_request()`; `expire_stale_friend_requests()` is service_role-only cron sugar, never the enforcement. Widens the `friend_requests` status CHECK with `'expired'` (looked up by definition, not by name) |
...
| `20260920150000_codex_silhouettes.sql` | `codex_silhouettes` — the Holotable ship view's top-down outlines (concept 2026-09-20): one row per `(channel, patch_version, build_number, kind, class_name)`, `kind in ('ship','weapon','component','item','armor')` (the uploader emits `item` for armour), `view_box` (1000-unit square), `path` (SVG path, holes as reversed-winding subpaths), `bbox`, `anchors` (**% of the viewBox** per port name, same transform as the path), `unresolved` (ports the mesh could not place), `meta` (source hull, tool version, SCC styling hints). Build-scoped like every `codex_*` table: viewer + anon `select`, service-role-only writes through `ingest-catalog` (`silhouettes` / `clear_silhouettes` ops, which degrade to `{ degraded: 'no_silhouettes_table' }` when the table is missing). The website only renders — `parseHoloSilhouette()` (`src/app/codex/holo-silhouette.ts`) returns `null` for anything invalid and the stage draws the §C3 placeholder; anchors are never projected client-side. Produced by `data-uploader/python/sc_extract/silhouette*.py` from the raw `.cga/.cgam` hull via cgf-converter (glTF Y-up rotated back to Cry axes) |
| `20260920160000_hangar_loadout_sharing.sql` | Hangar share links + follow/fork (Holotable concept rounds 2/3/6, red-teamed in `docs/concepts/…-build/wave1-redteam.md`). `hangar_ship_configs` gains `source_config_id` / `follows_owner` / `owner_user_id` / `forked_at`; `hangar_share_links` (owner-only RLS, `revoked_at` instead of DELETE — revoking stops NEW adoptions only, existing followers keep following until the owner deletes the config). Trigger `hangar_ship_configs_share_guard`: `follows_owner` may only go true→false, `source_config_id`/`owner_user_id` are immutable after insert and only settable under the transaction-local GUC `hangar.internal_write` (set by the RPCs), and an UPDATE of `name`/`role`/`loadout` on a following row auto-forks it (`follows_owner=false`, `forked_at=now()`) — the fork is irreversible by construction, not by UI. RPCs (SECURITY DEFINER, `social_actor()` + `is_suspended()` like every share path): `adopt_shared_loadout(token)` (refuses revoked/expired/self, one copy per `(user, source)`, creates the ship as `owned`), `hangar_follow_snapshot(config_id)` (pull-on-view refresh written inside the RPC, does not check the link), `peek_shared_loadout(token)` (granted to **anon** — read-only card: ship, loadout, name, role, channel, patch, owner name). All three return `owner_name` / `owner_updated_at` / `channel` / `patch_version` because `profiles` is self-read only |
| `20260921220000_profile_last_seen.sql` | Admin "last seen" was `auth.users.last_sign_in_at`, which only moves on an actual sign-in (never on a token refresh) — a member signed in for weeks read as inactive. Adds `profiles.last_seen_at`, written ONLY through `touch_last_seen()` (SECURITY DEFINER, pinned to `auth.uid()`, server-side 5-min throttle; suspended accounts deliberately not excluded — presence is a fact, not a privilege). `profiles_role_write_guard` is re-created a third time to freeze the column for raw sessions (`profiles_self_update` has no column list). `list_users_for_admin()` (dropped + re-created, RETURNS TABLE widened) gains `last_seen_at = greatest(sign-in, visit)`; `last_sign_in_at` stays for pre-migration clients. Client: `src/app/auth/presence.service.ts` — boot with a session, tab visible again, 15-min tick, always via `realClient` so an impersonating admin still counts |
| `20260925010000_codex_build_retention.sql` | Codex build retention (storage plan 2026-09-24, see `storage.md`). `prune_codex_builds(p_keep int default 2)` (SECURITY DEFINER, service-role only) deletes per channel every build older than the `p_keep` newest, never the `is_current` one; the `codex_*` children cascade. pg_cron job `codex-build-retention` runs it nightly at 03:30 UTC (not from ingest `finalize`: a cascade over ~500k rows can outlive the edge request, and a failed prune must not fail an ingest). Two builds, because the Holotable patch selector and the inline patch diff read the previous one. The four obsolete builds were deleted by hand beforehand + `VACUUM FULL`: 952 → 349 MB |
...
- `concept_pages` (`20260914160000`): `select` for admins (`public.is_admin()`), **no write policy for anybody** — the `concept-page` function writes with the service role after verifying its own ticket, the routine writes through the Management API. The browser never touches the table: the document and every bridge call go through the function.
...
- `moderation_actions` (`20260904020000`): `select` for the TARGET (`auth.uid() = user_id`) **or** an admin, no write policy for anybody, and — uniquely among the self-scoped tables — **no `is_approved()` gate**. That is deliberate: a suspended account has `is_approved() = false` by definition, and this is the table that has to tell it why.
- `loadout_shares` (`20260904020000`): `select` for the owner or the friend the row points at, no write policy (`create_loadout_link()` / `share_loadout_with_friend()` / `revoke_loadout_share()` only), plus the `is_approved()` gate. A LINK share has no recipient, so nobody but the owner can read the row — the token travels out of band, and reading it back out of the table is exactly what must not be possible. `hangar_role_loadouts` itself is untouched: `get_shared_loadout(token)` (granted to `anon`) and `list_loadouts_shared_with_me()` are SECURITY DEFINER projections, not a widened policy.
- `codex_silhouettes` (`20260920150000`): `select` for anon + authenticated (catalog data, like `codex_ships`), **no client write policy** — `ingest-catalog` writes with the service role.
- `hangar_share_links` (`20260920160000`): owner-only in all verbs plus the `is_approved()` gate; `anon` reaches a share only through `peek_shared_loadout(token)`, which returns the loadout card and never the row. `hangar_ship_configs` keeps its self-only policies — the follow/fork invariants live in the `hangar_ship_configs_share_guard` trigger, not in RLS, because the columns it protects are on a table the owner may otherwise edit freely.
...
- **`is_approved()`** — SECURITY DEFINER helper (`profiles.is_approved` for
  `auth.uid()`, `false` if no row). **Since `20260904020000` it also means
  "carries no ACTIVE suspension"** (`suspended_at` set and `suspended_until`
  null or in the future) — that is how an account suspension closes every
  gated table at once instead of touching a dozen policies. A
  `suspended_until` in the past lifts itself; nothing has to run.
  `profiles` and `moderation_actions` are deliberately **not** behind this
  gate, because a suspended session has to be able to read why it is being
  signed out (`my_account_status()`). Backs a RESTRICTIVE `FOR ALL` policy
...
| 3 | Trigger `profiles_role_write_guard` (BEFORE UPDATE, **SECURITY INVOKER** — it reads `current_user`) | Blocks direct `role`/`is_approved` writes from `authenticated`/`anon`; closes the old self-promotion hole in `profiles_self_update`. **Since `20260904020000` it also freezes `suspended_at`/`suspended_until`/`suspension_reason`** — `profiles_self_update` has no column list, so without that a suspended account would clear its own suspension with one `PATCH /rest/v1/profiles?id=eq.<self>`. **Any new privilege-relevant column on `profiles` must be added to this guard**, or the self-update policy hands it to the user |
...
| `concept-page` | Hosts interactive concept pages (admin feedback #224). `POST /ticket` (admin JWT → 12-h HMAC ticket, keyed with the service-role key, bound to one concept id) and, behind the ticket, `GET /:id` (the stored html with an injected fetch shim) plus the concept engine's bridge protocol (`/decisions`, `/reload`, `/heartbeat`, `/draft`; `/attachments` → 501). The ticket also carries the minting admin's uid; a real submit (`POST /decisions`, `submitted: true`) is echoed into the topic's thread as a HUMAN `admin_feedback_messages` row (`is_system=false`, `author_id=<uid>`, body from `echo.ts`) so the routine's queue picks the topic up — replays (same `submission_id` / identical payload) are skipped. Serves the document with `frame-ancestors <app origins>`, `no-store`, `noindex`. Reads/writes `concept_pages` with the service role. | `false` (iframe navigation cannot carry headers; auth is the ticket) |
