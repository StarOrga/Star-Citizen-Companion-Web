...
| `concept-page` | Hosts interactive concept pages (admin feedback #224). `POST /ticket` (admin JWT → 12-h HMAC ticket, keyed with the service-role key, bound to one concept id) and, behind the ticket, `GET /:id` (the stored html with an injected fetch shim) plus the concept engine's bridge protocol (`/decisions`, `/reload`, `/heartbeat`, `/draft`; `/attachments` → 501). Serves the document with `frame-ancestors <app origins>`, `no-store`, `noindex`. Reads/writes `concept_pages` with the service role. | `false` (iframe navigation cannot carry headers; auth is the ticket) |
